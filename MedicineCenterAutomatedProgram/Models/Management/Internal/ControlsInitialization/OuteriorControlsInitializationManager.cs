...
namespace MedicineCenterAutomatedProgram.Models.Management.Internal.ControlsInitialization
{
    public class OuteriorControlsInitializationManager
    {
        ...
        public static string HospitalAddressSelectedValueInitialization(string shiftHospitalAddressId, string shiftHospitalAddressValue)
        {
            ...
            {
                foreach (var city in DataResponseManager.CitiesJsonDataDeserialize($"SELECT DISTINCT(CityId) FROM HospitalAddresses, Cities, Streets, Houses WHERE HospitalAddressCityId = CityId AND HospitalAddressStreetId = StreetId AND HospitalAddressHouseId = HouseId AND CityTitle = '{UserDataFieldsViewManager.UserMainInteractionDataViewValueOptions(shiftHospitalAddressValue, "г. ", ",")}'"))
                {
                    foreach (var street in DataResponseManager.StreetsJsonDataDeserialize($"SELECT DISTINCT(StreetId) FROM HospitalAddresses, Cities, Streets, Houses WHERE HospitalAddressCityId = CityId AND HospitalAddressStreetId = StreetId AND HospitalAddressHouseId = HouseId AND CityId = {city.CityId} AND StreetTitle = '{UserDataFieldsViewManager.UserMainInteractionDataViewValueOptions(shiftHospitalAddressValue, "ул. ", ",")}'"))
                    {
                        foreach (var house in DataResponseManager.HousesJsonDataDeserialize($"SELECT DISTINCT(HouseId) FROM HospitalAddresses, Cities, Streets, Houses WHERE HospitalAddressCityId = CityId AND HospitalAddressStreetId = StreetId AND HospitalAddressHouseId = HouseId AND CityId = {city.CityId} AND StreetId = {street.StreetId} AND HouseNumber = '{UserDataFieldsViewManager.UserMainInteractionDataViewValueOptions(shiftHospitalAddressValue, "д. ", " ")}'"))
                        {
